...
p.add_argument('--word_path', required=True)
p.add_argument('--sen_path', required=True)
p.add_argument('--dial_path', required=True)
config=p.parse_args()
...
import os
path = config.dial_path[:-9] + '원천데이터/분할대화'
os.mkdir(path)
...
for fileno in range(len(file)):
    ...
    for i in range(len(start_time[fileno])):
        ...
        audio_chunk.export(config.dial_path[:-9] + "원천데이터/분할대화/{}_part{}.wav".format(file[fileno][:-4], i), format="wav")
...
for fileno in range(0, len(file)):
    for i in range(0, len(text[fileno])):
        ...
        split_filepath.append(config.dial_path[:-9] + "원천데이터/분할대화/" + file_path[fileno][-30:-4] + "_part{}.wav".format(i))
